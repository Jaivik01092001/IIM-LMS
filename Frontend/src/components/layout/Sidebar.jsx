...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  useEffect(() => {
    ...
    const activeDropdown = menuItems.find(item =>
      item.isDropdown && isSubmenuActive(item.submenu)
    );
        ...
        <nav className="sidebar-nav">
          <ul className="sidebar-menu">
            {getMenuItems().map((item) => (
              <li key={item.id} className={`menu-item ${item.isDropdown ? 'dropdown-menu' : ''}`}>
                {item.isDropdown ? (
                  <>
                    <button
                      className={`dropdown-toggle ${openDropdown === item.id ? 'open' : ''} ${isSubmenuActive(item.submenu) ? 'active' : ''}`}
                    ...
                    <div className={`dropdown-content ${openDropdown === item.id ? 'open' : ''}`}>
