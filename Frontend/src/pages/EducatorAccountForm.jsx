...
const EducatorAccountForm = () => {
  ...
  const filteredRoles = roles
    ? roles.filter((role) => {
        ...
        console.log(
          `Role: "${role.name}", lowercase: "${roleLowerCase}", excluded: ${isExcluded}`
        );
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.profileImage) {
      console.log(
        "Adding profile image to form data:",
    ...
    if (isEditMode) {
      ...
      console.log(
        "Using createThunk:",
        isAdmin ? "createAdminEducatorThunk" : "createUniversityEducatorThunk"
      );
